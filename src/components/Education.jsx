    ...
    <section id="education" className="relative py-20">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="relative flex justify-center items-center mb-12">
          <h2 className="text-3xl md:text-5xl font-extrabold text-white tracking-wider [text-shadow:0_0_25px_#7C3AED,0_0_10px_#4C1D95] text-center">Education</h2>
        ...
        <div className="mt-8 relative">
          ...
          {education.map((edu, index) => (
            <div key={edu.id} className={`mb-8 flex flex-col md:flex-row justify-between items-center w-full ${index % 2 === 0 ? '' : 'md:flex-row-reverse'}`}>
              ...
              <div className="order-1 bg-gray-800 rounded-lg shadow-[0_0_30px_rgba(124,58,237,0.3)] border border-purple-800/40 w-full md:w-5/12 px-6 py-4 transform hover:scale-105 transition-transform duration-300">
                <h3 className="mb-3 font-bold text-violet-400 text-xl flex justify-between items-start">
                  {edu.degree}
                  <span className="md:hidden text-xs bg-purple-900/50 text-purple-200 px-2 py-1 rounded-full border border-purple-700/50">#{index + 1}</span>
                </h3>
                <p className="text-sm leading-snug tracking-wide text-gray-300 text-opacity-100">{edu.school} | {edu.location} | {edu.year}</p>
              </div>
            </div>
          ))}
